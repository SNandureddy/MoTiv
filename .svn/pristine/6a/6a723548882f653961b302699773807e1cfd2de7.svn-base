...
extension APIService {
    ...
    private func requestNormal(normalSuccess: @escaping APIServiceSuccessCallback, normalFailure: @escaping APIServiceFailureCallback) {
        ...
        //Create Request
        Alamofire.request(path, method: resource.method, parameters: resource.parameters, encoding: URLEncoding.default, headers: resource.headers).validate().responseJSON(completionHandler: { (response) in
...
//                }
                ...
                if let array = value as? JSONArray {
                    let arrData =  try! JSONSerialization.data(withJSONObject: array, options: .prettyPrinted)
                    uploadData.append(arrData)
                    uploadData.append("\r\n".data(using: String.Encoding.utf8)!)
//                    uploadData.append("\(arrData)\r\n")
                }
                else {
