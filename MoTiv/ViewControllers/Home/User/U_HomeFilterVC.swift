...
class U_HomeFilterVC: BaseVC {
    ...
    static var searchSliderValue: Int? = 0
    static var searchFilter : String? = ""
    static var searchSelectedDate : Date?
    static var searchSelectedPublicArray = [String]()
    static var searchSelectedMusicArray = [String]()
    static var searchRemainingPublicArray = [String]()
    static var searchRemainingMusicArray = [String]()
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if previousScreen == .main {
            ...
            dict = [APIKeys.kFilter : U_HomeFilterVC.filter ?? "", APIKeys.kLat : U_HomeFilterVC.sliderValue == 0 ? "" : U_HomeFilterVC.currentLatitude ?? 0.0, APIKeys.kLong : U_HomeFilterVC.sliderValue == 0 ? "" : U_HomeFilterVC.currentLongitude ?? 0.0, APIKeys.kMiles : U_HomeFilterVC.sliderValue == 0 ? "" : U_HomeFilterVC.sliderValue ?? 0, APIKeys.kPublicInterest : U_HomeFilterVC.selectedPublicArray.joined(separator:","), APIKeys.kMusicInterest : U_HomeFilterVC.selectedMusicArray.joined(separator:","), APIKeys.kEventDate : filteredDate ?? ""] as JSONDictionary
        } else {
            U_HomeFilterVC.searchSelectedPublicArray = (selectedPublicCategoryArray.compactMap({$0.id})).map{String($0)}
            U_HomeFilterVC.searchSelectedMusicArray = (selectedMusicCategoryArray.compactMap({$0.id})).map{String($0)}
            U_HomeFilterVC.searchRemainingPublicArray = (userPublicCategoryArray.compactMap({$0.id})).map{String($0)}
            U_HomeFilterVC.searchRemainingMusicArray = (userMusicCategoryArray.compactMap({$0.id})).map{String($0)}
            dict = [APIKeys.kFilter : U_HomeFilterVC.searchFilter ?? "", APIKeys.kLat : U_HomeFilterVC.searchSliderValue == 0 ? "" : U_HomeFilterVC.currentLatitude ?? 0.0, APIKeys.kLong : U_HomeFilterVC.searchSliderValue == 0 ? "" : U_HomeFilterVC.currentLongitude ?? 0.0, APIKeys.kMiles : U_HomeFilterVC.searchSliderValue == 0 ? "" : U_HomeFilterVC.searchSliderValue ?? 0, APIKeys.kPublicInterest : U_HomeFilterVC.searchSelectedPublicArray.joined(separator:","), APIKeys.kMusicInterest : U_HomeFilterVC.searchSelectedMusicArray.joined(separator:","), APIKeys.kEventDate : filteredDate ?? ""] as JSONDictionary
        }
        if previousScreen == .main {
            if U_HomeFilterVC.filter == "" {
                delegate?.setFilteredData?(object: JSONDictionary())
            } else {
                delegate?.setFilteredData?(object: dict)
            }
        } else {
            if U_HomeFilterVC.searchFilter == "" {
                delegate?.setFilteredData?(object: JSONDictionary())
            } else {
                delegate?.setFilteredData?(object: dict)
            }
    ...
    private func customizeUI() {
        ...
        if previousScreen == .main {
            ...
            eventSearchLabel.text = "I'M LOOKING FOR AN EVENT WITHIN \(U_HomeFilterVC.searchSliderValue ?? 0) MILES"
            dateTextField.text = U_HomeFilterVC.searchSelectedDate != nil ? U_HomeFilterVC.searchSelectedDate?.stringFromDate(format: .longMDYDate, type: .local) : ""
        }
    ...
    private func resetData() {
        slider.value = 0.0
        if previousScreen == .main {
            U_HomeFilterVC.sliderValue = 0
            U_HomeFilterVC.filter = ""
            U_HomeFilterVC.selectedDate = nil
        } else {
            U_HomeFilterVC.searchSliderValue = 0
            U_HomeFilterVC.searchFilter = ""
            U_HomeFilterVC.searchSelectedDate = nil
        }
    ...
    private func handleCategories() {
        if previousScreen == .main {
            ...
            if U_HomeFilterVC.searchSelectedPublicArray.count != 0 {
                for interest in U_HomeFilterVC.searchSelectedPublicArray {
                    if let index = CommonVM.shared.publicArray.firstIndex(where: {$0.id == Int(interest)}) {
                        selectedPublicCategoryArray.append( CommonVM.shared.publicArray[index])
                    }
                }
                for interest in U_HomeFilterVC.searchRemainingPublicArray {
                    if let index = CommonVM.shared.publicArray.firstIndex(where: {$0.id == Int(interest)}) {
                        userPublicCategoryArray.append( CommonVM.shared.publicArray[index])
                    }
                }
            ...
            if U_HomeFilterVC.searchSelectedMusicArray.count != 0 {
                for interest in U_HomeFilterVC.searchSelectedMusicArray {
                    if let index = CommonVM.shared.musicArray.firstIndex(where: {$0.id == Int(interest)}) {
                        selectedMusicCategoryArray.append( CommonVM.shared.musicArray[index])
                    }
                }
                for interest in U_HomeFilterVC.searchRemainingMusicArray {
                    if let index = CommonVM.shared.publicArray.firstIndex(where: {$0.id == Int(interest)}) {
                        userMusicCategoryArray.append( CommonVM.shared.publicArray[index])
                    }
                }
    ...
    func setupSlider(){
        ...
        if previousScreen == .main {
            ...
            slider.value = Float(U_HomeFilterVC.searchSliderValue ?? 0)
        }
    ...
    @IBAction func sliderAction(_ sender: Any) {
        if previousScreen == .main {
            ...
            U_HomeFilterVC.searchFilter = "1"
            U_HomeFilterVC.searchSliderValue = Int(Double(slider.value).changeToInt) ?? 0
            eventSearchLabel.text = "I'M LOOKING FOR AN EVENT WITHIN \(U_HomeFilterVC.searchSliderValue ?? 0) MILES"
        }
    }
...
extension U_HomeFilterVC: UITextFieldDelegate {
    ...
    func textFieldDidBeginEditing(_ textField: UITextField) {
        ...
        if previousScreen == .main {
            ...
            self.setDatePicker(textField: textField, mode: .date, maxdate: nil, date: U_HomeFilterVC.searchSelectedDate, minDate: Date())
            dateTextField.text = U_HomeFilterVC.searchSelectedDate?.stringFromDate(format: .longMDYDate, type: .local)
...
extension U_HomeFilterVC: PickerDelegate {
    ...
    func didSelectDatePicker(date: Date) {
        if previousScreen == .main {
            ...
            U_HomeFilterVC.searchFilter = "1"
            U_HomeFilterVC.searchSelectedDate = date
        }
